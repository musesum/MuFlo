...
public extension SIMD2<Float> {
    ...
    init(_ size: CGSize) {
        self = SIMD2<Float>(Float(size.width), Float(size.height))
    }
    init(_ point: CGPoint) {
        self = SIMD2<Float>(Float(point.x), Float(point.y))
    }
}
...
public extension SIMD4<Float> {
    ...
    init(_ rect: CGRect) {
        self = SIMD4<Float>(Float(rect.minX),
                            Float(rect.minY),
                            Float(rect.width),
                            Float(rect.height))
    }
...
public extension simd_float4x4 {
    ...
    func digits(_ range: Int = -1) -> String {
        return "(\(columns.0.digits(range)), \(columns.1.digits(range)), \(columns.2.digits(range)), \(columns.3.digits(range)))"
    }
}
...
public func translate4x4(_ t: vector_float4) -> matrix_float4x4 {
