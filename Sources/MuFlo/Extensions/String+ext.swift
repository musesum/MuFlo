...
public extension String {
    ...
    func splitWild(_ wild: String) -> (String, String, String) {
        ...
        // get remaining chars for suffix
    ...
    /// append new value to prior value separated by `, `
    mutating func commaPlus(_ str: String?) {

        guard let str, str != "" else { return }
        if      self == ""            { self = str }
        else if self.hasSuffix(", ")  { self += str }
        else if self.hasSuffix(",")   { self += " " + str }
        else                          { self += ", " + str }
    }

    /// append new value to prior value separated by `, `
    mutating func trimCommaSpace() {
        var trim = self
        while trim.last == " " || trim.last == "," {
            trim.removeLast()
        }
        self = trim
    }
    ...
    func hasChar(in set: String) -> Bool {
        for char in self {
    ...
    var IntegerValue: Int? { Int(self.filter { $0.isNumber }) }
