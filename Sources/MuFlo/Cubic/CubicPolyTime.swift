...
class CubicPolyVal {
    ...
    func addTweVals(_ twes: [Double],
        ...
        func verifyValCount() {
            if valCount == 0 {
                ...
                PrintLog("⁉️ CubicPolyVal: mismatched vals.count: \(vals.count) != valCount:  \(valCount)")
            }
        }
    }
    ...
    func getTweenNow(_ time: TimeInterval) -> [Double] {
        ...
        let timeStart = timeSegmentStart(time)
        let timeCount = timeVals.count-1
        let ii = (timeStart,
                  min(timeStart+1, timeCount),
                  min(timeStart+2, timeCount),
                  min(timeStart+3, timeCount))
