...
pipe(on 1) {
    ...
    cell(on 1) {
        ...
        out(tex)
        rule(on 1) {
            slide(on 1, -> (slide(on 0), zha(on 0), ave(on 0), fade(on 0), melt(on 0), tunl(on 0), fred(on 0))) {
                version(buf, x 0…7 : 3, -> slide(on 1))
                loops(buf, z 0)
            }
            zha(on 0, -> (slide(on 0), zha(on 0), ave(on 0), fade(on 0), melt(on 0), tunl(on 0), fred(on 0))) {
                version(buf, x 0…6 : 2, -> zha(on 1))
                loops(buf, z 10)
            }
            ave(on 0, -> (slide(on 0), zha(on 0), ave(on 0), fade(on 0), melt(on 0), tunl(on 0), fred(on 0))) {
                version(buf, x 0…1 : 0.5, -> ave(on 1))
                loops(buf, y 0…99~0)
            }
            fade(on 0, -> (slide(on 0), zha(on 0), ave(on 0), fade(on 0), melt(on 0), tunl(on 0), fred(on 0))) {
                version(buf, x 1…3 : 1.2, -> fade(on 1))
                loops(buf, y 0…32~0)
            }
            melt(on 0, -> (slide(on 0), zha(on 0), ave(on 0), fade(on 0), melt(on 0), tunl(on 0), fred(on 0))) {
                version(buf, x 0…1 : 0.5, -> melt(on 1))
                loops(buf, y 0…32~0)
            }
            tunl(on 0, -> (slide(on 0), zha(on 0), ave(on 0), fade(on 0), melt(on 0), tunl(on 0), fred(on 0))) {
                version(buf, x 0…5 : 1, -> tunl(on 1))
                loops(buf, y 0…32~0)
            }
            fred(on 0, -> (slide(on 0), zha(on 0), ave(on 0), fade(on 0), melt(on 0), tunl(on 0), fred(on 0))) {
                version(buf, x 0…4 : 4, -> fred(on 1))
                loops(buf, y 0…32~0)
            }
        }
    }
    ...
    render(on 1) {
        in(tex, <- tile.out)
        map(on 1) {
            flat(on 1, -> (flat(on 0), cube(on 0)))
            cube(on 1, -> (flat(on 0), cube(on 0))) {
                cudex(tex)
                mix(buf, x 0…1 : 1)
            }
        }
...
model {
    canvas(columns 2) {
        color(xy, x 0…1, y 0…1, <> (pipe.color.plane, sky.color.xfade), ^- sky.main.anim)
        brush(val, x 1_255~255, <> sky.draw.brush.index)
        repeat(xy, x -1…1~0, y -1…1~0, <> pipe.tile.repeat, -> (midi.cc.skypad.repeatX(val x), midi.cc.skypad.repeatY(val y)), ^- sky.main.anim)
        mirror(xy, x 0…1~0, y 0…1~0, <> pipe.tile.mirror, ^- sky.main.anim)
    ...
    plato(columns 2) {
        ...
        cubemap(tog, x 1, <> pipe.render.map.cube.mix)
    ...
    cell(columns 2) {
        slide(seg, x 0_7~3, <> pipe.cell.rule.slide.version)
        zha(seg, x 0_6~2, <> pipe.cell.rule.zha.version)
        ave(val, x 0…1~0.5, <> pipe.cell.rule.ave.version)
        fade(val, x 0.2…2~1, <> pipe.cell.rule.fade.version)
        melt(val, x 0…1~0.5, <> pipe.cell.rule.melt.version)
        tunl(seg, x 0_5~1, <> pipe.cell.rule.tunl.version)
        fred(seg, x 0_4~4, <> pipe.cell.rule.fred.version)
    }
    camera {
        stream(tog, x 0, <> (pipe.camera(on x), cubemap(x 0)))
        front(tog, x 1, <> pipe.camera.front)
        cubemap(tog, x 1, <> pipe.render.map.cube.mix)
        mix(xy, x 1, y 0…1~0, <> (pipe.camix.mix, pipe.cell.rule.slide.loops(y), pipe.cell.rule.zha.loops(y), pipe.cell.rule.ave.loops(y), pipe.cell.rule.fade.loops(y), pipe.cell.rule.melt.loops(y), pipe.cell.rule.tunl.loops(y), pipe.cell.rule.fred.loops(y)))
...
midi { // musical instrument device interface
    input { // midi input
        note { // note on/off from 0 thru 127
            on(num 0_127, velo 0_127, chan 1_32, port 1_16, time, -> draw.dot.on)
            off(num 0_127, velo 0_127, chan 1_32, port 1_16, time, -> draw.dot.off)
        }
        ...
        nrpn(num 0_16383, val 0…1, chan, time, -> (skypad.plane, skypad.fade))
    ...
    cc {
        dispatch(<> (skypad.zoom, skypad.convex, skypad.colorY, skypad.camix, skypad.fade, skypad.plane, skypad.shiftX, skypad.shiftY, skypad.repeatX, skypad.repeatY, roli.lightpad.x, roli.lightpad.y, roli.lightpad.z))
        skypad {
            ...
            repeatY(cc == 15, val 0_127, <>model.canvas.tile.repeat(y = val)) // skypad˚. >> output.note.on(num val)
        }
        roli {
            lightpad {
